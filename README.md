...
*   Set slide layouts: `<!-- layout: title -->`, `<!-- layout: navigation -->`, `<!-- layout: 2-column -->`, `<!-- layout: 3-column -->`, `<!-- layout: title-image -->`, `<!-- layout: image-only -->`, or `<!-- layout: stacked -->`.
...
7.  **Image Only (`image-only`)**: Same full-stage image as title-image, but the Markdown `#` heading is used only for the slide tray label (hidden on the stage).
8.  **Stacked (`stacked`)**: Keeps title and content on top and places the image below, filling remaining height (disables auto-split).
